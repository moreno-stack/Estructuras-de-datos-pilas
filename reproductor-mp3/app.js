...
class NodoCancion {
    ...
    artista;
    urlAudio;
    portada; // URL de la imagen (de iTunes o genérica)
    siguiente;
    anterior;
    constructor(titulo, artista, urlAudio, portada) {
        this.titulo = titulo;
...
class PlaylistDoble {
    ...
    cancionActual = null;
    longitud = 0;
    agregarAlFinal(titulo, artista, urlAudio, portada) {
        ...
        this.longitud++;
        if (this.longitud === 1)
            this.cancionActual = this.cabeza;
    }
    agregarAlInicio(titulo, artista, urlAudio, portada) {
        const nuevoNodo = new NodoCancion(titulo, artista, urlAudio, portada);
        if (!this.cabeza) {
            this.cabeza = nuevoNodo;
            this.cola = nuevoNodo;
    ...
    agregarEnPosicion(titulo, artista, urlAudio, portada, posicion) {
        if (posicion <= 0) {
            this.agregarAlInicio(titulo, artista, urlAudio, portada);
            return;
        }
        if (posicion >= this.longitud) {
            this.agregarAlFinal(titulo, artista, urlAudio, portada);
            return;
        }
        const nuevoNodo = new NodoCancion(titulo, artista, urlAudio, portada);
...
const ulPlaylist = document.getElementById('ul-playlist');
...
const imgPortadaActual = document.getElementById('img-portada-actual');
const txtTituloActual = document.getElementById('txt-titulo-actual');
const txtArtistaActual = document.getElementById('txt-artista-actual');
const barraProgreso = document.getElementById('barra-progreso');
// --- GESTIÓN DE POSICIÓN ---
selectPosicion.addEventListener('change', () => {
    inputIndice.style.display = selectPosicion.value === 'cualquiera' ? 'block' : 'none';
});
function obtenerPosicionDeseada() {
    const metodo = selectPosicion.value;
    let indice = -1;
    if (metodo === 'cualquiera') {
        const val = parseInt(inputIndice.value);
        if (!isNaN(val) && val > 0)
            indice = val - 1; // Convertir posición humana (1,2..) a índice (0,1..)
    }
    return { metodo, indice };
}
function procesarIngresoCancion(titulo, artista, url, portada) {
    ...
    if (config.metodo === 'final') {
        ...
        if (config.indice === -1) {
            alert("Ingresa un número de posición válido");
            return;
        }
        miPlaylist.agregarEnPosicion(titulo, artista, url, portada, config.indice);
    }
    if (miPlaylist.longitud === 1)
        cargarCancionEnReproductor(false); // Carga la data pero no le da play
    actualizarUI();
}
// --- ACTUALIZAR INTERFAZ ---
function actualizarUI() {
    ...
    while (actual) {
        ...
        if (actual === miPlaylist.cancionActual)
...
btnBuscar.addEventListener('click', async () => {
    ...
    try {
        ...
        datos.results.forEach((cancion) => {
            ...
            `;
            const btnAdd = div.querySelector('.btn-add-mini');
            btnAdd.onclick = () => {
...
document.getElementById('btn-agregar-local')?.addEventListener('click', () => {
    const inputTitulo = document.getElementById('input-titulo-local');
    const inputArtista = document.getElementById('input-artista-local');
    const inputArchivo = document.getElementById('input-archivo');
    if (!inputTitulo.value || !inputArtista.value || !inputArchivo.files || inputArchivo.files.length === 0) {
        alert("Completa todos los campos locales y selecciona un archivo .mp3");
        return;
    }
...
function cargarCancionEnReproductor(autoPlay = false) {
...
document.getElementById('barra-volumen')?.addEventListener('input', (e) => {
    reproductorAudio.volume = parseInt(e.target.value) / 100;
});
